...
var responseData = Drupal.settings.fido;
var fidoData = {};
var fidoSettings = Drupal.settings.fidoSettings;

jQuery.each(responseData, function(fieldName, field) {
  var rows = [];
  jQuery.each(field.rows, function(index, row) {
    if (fieldName == "InsideTemp" || fieldName == "OutsideTemp") {
      rows.push(parseInt(row.value));
    }
    if (fieldName == "Sunlight") {
      rows.push(parseInt(row.value*100));
    }
  });
  fidoData[fieldName] = rows;
});

//var theDataTest = [2,4,2,6,3,34]

    var chart;
    var pointStart = Date.UTC(fidoSettings.pointStart[0], fidoSettings.pointStart[1]-1, fidoSettings.pointStart[2]);
    var pointInterval =  1000 * 60; //60 * 24 * 30 * 12 * 10; // 3600000 by minute or by hour   
    var theSeries = [];

        jQuery.each(fidoData, function(fieldName, fieldData){
...
jQuery(document).ready(function() {
  var chart;
  var pointStart = Date.UTC(2000, 4, 01);
  var pointInterval = 24 * 3600 * 1000; // by minute or by hour
  var dataFields = {
    "outsideTemp" : {
      "show": true,
      "data": [88,99,44,22,11,55]
    },
    "insideTemp" : {"show": false},
    "Sunlight" : {"show": false},
    "CumulativeEnergyUse" : {"show": false}
  }
...
/*  $.each(dataFields, function(fieldName, field) {
    if(field.
...
/*
jQuery.ajax({
    "url": 'http://10.1.10.28:80/fido99/_design/docs/_view/data?startkey="2012-05-04--12:00:00"&endkey="2012-05-04--12:15:00"',
    "type": "GET",
    "dataType": "jsonp",
